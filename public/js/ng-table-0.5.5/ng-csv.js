    ...
    this.stringify = function (data, options) {
      ...
      var dataPromise = $q.when(data).then(function (responseData) {
        ...
        if (angular.isDefined(options.header) && options.header) {
          ...
          encodingArray = [];
          angular.forEach(options.header, function(item){
            var headerGroup = [];
            angular.forEach(item, function (title, key) {
              this.push(that.stringifyField(title, options));
            }, headerGroup);
            encodingArray.push(headerGroup.join(options.fieldSep ? options.fieldSep : ","));
          });

          angular.forEach(encodingArray, function(item){
            headerString += item + EOL;
          });
